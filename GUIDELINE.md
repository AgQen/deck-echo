...
게임은 PWA(Progressive Web App)로 설치 가능합니다. 브라우저 주소창에서 `https://agqen.github.io/deck-echo/` 를 연 후:

### PC (Chrome / Edge)
- 주소창 오른쪽에 ⊕ 아이콘 또는 "설치" 버튼이 나옵니다 → 클릭
- 바탕화면과 시작 메뉴에 앱처럼 등록됩니다 (창이 따로 뜸, 주소창 없음)

### 안드로이드 (Chrome)
- 메뉴(⋮) → "홈 화면에 추가" 또는 "앱 설치"
- 홈에 아이콘이 생기고 누르면 전체화면으로 실행
...
### Windows 바로가기 (URL 파일)
저장소의 [DECK-ECHO.url](DECK-ECHO.url) 파일을 더블클릭하면 기본 브라우저로 게임이 열립니다.
바탕화면에 복사해두면 일반 프로그램 바로가기처럼 사용 가능.
